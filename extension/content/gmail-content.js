(() => {
  ...
  if (window.__mailShieldGmailLoaded) {
    return;
  }
  window.__mailShieldGmailLoaded = true;
  ...
  function renderResult(result) {
    ...
    const actionsHtml = recommendedActions.length
      ? `<p class="mailshield-actions"><strong>Recommended protections:</strong> ${recommendedActions.join(", ")}<br><em>(Display only in this build — active blocking is a stretch-goal feature.)</em></p>`
      : "";
  ...
  function renderError(message) {
    renderPanel(`
      <div class="mailshield-header mailshield-risk-error">
  ...
  function handleAnalyzeClick({ auto = false } = {}) {
    ...
    const emailData = window.__mailshieldExtract
      ? window.__mailshieldExtract()
      : null;
